...
def collaborative_filtering(input_array, n_neighbors,
    ...
    """
    kNN = KNearestNeighbor(n_neighbors, distance_measure, aggregator)
    ...
    for x in range(input_array.shape[0]):
        ...
        kNN.fit(input_array, targets)
        preds = kNN.predict(row, ignore_first=True)
        for y in range(np.size(zeros)):
            imputed_array[x, zeros[y]] = preds[y]
