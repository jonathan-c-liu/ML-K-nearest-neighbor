...
class KNearestNeighbor():    
    ...
    def predict(self, features, ignore_first=False):
        ...
        """
        preds = np.zeros((features.shape[0], self.targets.shape[1]))
        if self.distance_measure == 'euclidean':
            dists = euclidean_distances(features, self.features)
        elif self.distance_measure == 'manhattan':
            dists = manhattan_distances(features, self.features)
        elif self.distance_measure == 'cosine':
            dists = cosine_distances(features, self.features)

        for x in range(dists.shape[0]):
            if ignore_first:
                NN_index = np.argsort(dists[x, :])[1:self.n_neighbors + 1]
            else:
                NN_index = np.argsort(dists[x, :])[:self.n_neighbors]
            targets = self.targets[NN_index, :]

            if self.aggregator is 'mode':
                modes = np.zeros(targets.shape[1])
                for y in range(targets.shape[1]):
                    modes[y] = mode(targets[:, y])
                preds[x, :] = modes
            elif self.aggregator == 'mean':
                preds[x, :] = np.mean(targets, axis=0)
            elif self.aggregator == 'median':
                preds[x, :] = np.median(targets, axis=0)
